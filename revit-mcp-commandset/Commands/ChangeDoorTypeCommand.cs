...
{
    public class ChangeDoorTypeCommand : ExternalEventCommandBase
    {
        private ChangeFamilyInstanceTypeEventHandler _handler => (ChangeFamilyInstanceTypeEventHandler)Handler;

        public override string CommandName => "change_door_type";

        public ChangeDoorTypeCommand(UIApplication uiApp)
            : base(new ChangeFamilyInstanceTypeEventHandler(Autodesk.Revit.DB.BuiltInCategory.OST_Doors), uiApp)
        {
        }
        ...
        public override object Execute(JObject parameters, string requestId)
        {
            try
            {
                ...
                if (RaiseAndWaitForCompletion(15000))
                    ...
                    throw new TimeoutException("Change door type timed out");
            }
            catch (Exception ex)
            {
                throw new Exception($"Change door type failed: {ex.Message}");
            }
        }
    }
}
